...
public class UtilEjml {
     ...
     * Default tolerance.
     */
    public static float TOLERANCE = 1e-8f;

    public static float EPS = (float)Math.pow(2,-52);

    public static boolean isUncountable( float val ) {
        return Float.isNaN(val) || Float.isInfinite(val);
    }

    public static void memset( float[] data , float val ) {
        for( int i = 0; i < data.length; i++ ) {
            data[i] = val;
        }
    }

    public static void memset( float[] data , float val , int length ) {
     ...
     */
    public static DenseMatrix32F parseMatrix( String s , int numColumns )
    {
        ...
        // covert it from string to floats
   ...
   /* public static Integer[] sortByIndex( final float []data , int size ) {
        ...
        Arrays.sort(idx, new Comparator<Integer>() {
            @Override public int compare(final Integer o1, final Integer o2) {
                return Float.parseFloat(data[o1], data[o2]);
            }
        });
