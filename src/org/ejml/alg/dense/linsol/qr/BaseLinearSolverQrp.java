...
public abstract class BaseLinearSolverQrp extends LinearSolverAbstract {
    ...
    QRPDecomposition<DenseMatrix32F> decomposition;

    // if true then only the basic solution will be found
    protected boolean norm2Solution;

    protected DenseMatrix32F Y = new DenseMatrix32F(1,1);
    protected DenseMatrix32F R = new DenseMatrix32F(1,1);
    
    // stores sub-matrices inside the R matrix
    protected DenseMatrix32F R11 = new DenseMatrix32F(1,1);
    
    // store an identity matrix for computing the inverse
    protected DenseMatrix32F I = new DenseMatrix32F(1,1);

    // rank of the system matrix
    protected int rank;

    protected LinearSolver<DenseMatrix32F> internalSolver = LinearSolverFactory.leastSquares(1, 1);

    // used to compute optimal 2-norm solution
    private DenseMatrix32F W = new DenseMatrix32F(1,1);
     ...
     */
    protected BaseLinearSolverQrp(QRPDecomposition<DenseMatrix32F> decomposition,
                                  boolean norm2Solution)
    {
        ...
        if( internalSolver.modifiesA() )
            internalSolver = new LinearSolverSafe<DenseMatrix32F>(internalSolver);
    }

    @Override
    public boolean setA(DenseMatrix32F A) {
        _setA(A);
     ...
     */
    protected void upgradeSolution( DenseMatrix32F X ) {
        DenseMatrix32F z = Y; // recycle Y
    ...
    @Override
    public void invert(DenseMatrix32F A_inv) {
    ...
    public QRPDecomposition<DenseMatrix32F> getDecomposition() {
