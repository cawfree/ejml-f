 ...
 * @author Peter Abeles
 */
public class SolvePseudoInverseSvd implements LinearSolver<DenseMatrix32F> {

    // Used to compute pseudo inverse
    private SingularValueDecomposition<DenseMatrix32F> svd;

    // the results of the pseudo-inverse
    private DenseMatrix32F pinv = new DenseMatrix32F(1,1);

    // relative threshold used to select singular values
    private float threshold = UtilEjml.EPS;
    ...
    public boolean setA(DenseMatrix32F A) {
        ...
        DenseMatrix32F U_t = svd.getU(null,true);
    ...
    public SingularValueDecomposition<DenseMatrix32F> getDecomposer() {
        return svd;
    }
}
