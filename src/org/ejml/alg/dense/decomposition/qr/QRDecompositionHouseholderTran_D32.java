 ...
 * </p>
 *
 * @see QRDecompositionHouseholder_D32
 *
 * @author Peter Abeles
 */
...
public class QRDecompositionHouseholderTran_D32 implements QRDecomposition<DenseMatrix32F> {
    ...
    public void setExpectedMaxSize( int numRows , int numCols ) {
        ...
        if( QR == null ) {
            QR = new DenseMatrix32F(numCols,numRows);
     ...
     */
    public void applyQ( DenseMatrix32F A ) {
     ...
     */
    public void applyTranQ( DenseMatrix32F A ) {
    ...
    public DenseMatrix32F getR(DenseMatrix32F R, boolean compact) {
        if( R == null ) {
            if( compact ) {
                R = new DenseMatrix32F(minLength,numCols);
            } else
                R = new DenseMatrix32F(numRows,numCols);
        } else {
            if( compact ) {
    ...
    protected void householder( final int j )
    {
        ...
        final float max = QrHelperFunctions.findMax(QR.data,startQR,numRows-j);
