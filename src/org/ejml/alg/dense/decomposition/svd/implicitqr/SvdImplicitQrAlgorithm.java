...
public class SvdImplicitQrAlgorithm {
    ...
    protected DenseMatrix32F Ut;
    protected DenseMatrix32F Vt;
...
//    SimpleMatrix B;
    ...
    public void setUt(DenseMatrix32F ut) {
        Ut = ut;
    }
    ...
    public void setVt(DenseMatrix32F vt) {
        Vt = vt;
    }
...
//        return Math.abs(diag[i]) <= maxValue* UtilEjml.EPS;
     ...
     */
    protected void updateRotator( DenseMatrix32F Q , int m, int n, float c, float s) {
...
//            System.out.println();
    ...
    {
//        float gamma = Math.sqrt(rise*rise + run*run);
//
//        c = rise/gamma;
//        s = run/gamma;
        ...
        // See page 384 of Fundamentals of Matrix Computations 2nd
          if( Math.abs(rise) < Math.abs(run)) {
              ...
              float bottom = (float)Math.sqrt(1.0d+k*k);
              s = 1.0f/bottom;
              c = k/bottom;
          } else {
              float t = run/rise;
              float bottom = (float)Math.sqrt(1.0d + t*t);
              c = 1.0f/bottom;
              s = t/bottom;
          }
    }
...
//            System.out.println();
    ...
    public void exceptionShift() {
        ...
        float angle = 2.0f*(float)Math.PI*(rand.nextFloat()-0.5f)*mag;
