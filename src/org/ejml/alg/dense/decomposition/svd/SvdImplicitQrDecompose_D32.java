...
public class SvdImplicitQrDecompose_D32 implements SingularValueDecomposition<DenseMatrix32F> {
     ...
     */
    public SvdImplicitQrDecompose_D32(boolean compact, boolean computeU, boolean computeV,
    ...
    @Override
    public DenseMatrix32F getU( DenseMatrix32F U , boolean transpose) {
        if( !prefComputeU )
        ...
        if( transpose ) {
            ...
            if( U == null )
                U = new DenseMatrix32F(Ut.numCols,Ut.numRows);
    ...
    @Override
    public DenseMatrix32F getV( DenseMatrix32F V , boolean transpose ) {
        if( !prefComputeV )
        ...
        if( transpose ) {
            ...
            if( V == null )
                V = new DenseMatrix32F(Vt.numCols,Vt.numRows);
    ...
    private boolean bidiagonalization(DenseMatrix32F orig) {
...
//        System.out.println("  compute UV "+(pointB-pointA)+"  QR = "+(pointC-pointB));
    ...
    private boolean setup(DenseMatrix32F orig) {
        ...
        if( canUseTallBidiagonal && numRows > numCols * 2 && !computeU ) {
            if( bidiag == null || !(bidiag instanceof BidiagonalDecompositionTall_D32) ) {
                bidiag = new BidiagonalDecompositionTall_D32();
            }
        } else if( bidiag == null || !(bidiag instanceof BidiagonalDecompositionRow_D32) ) {
            bidiag = new BidiagonalDecompositionRow_D32();
        }
