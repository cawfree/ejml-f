...
public class BidiagonalDecompositionTall_D32
        implements BidiagonalDecomposition<DenseMatrix32F>
{
    QRPDecomposition<DenseMatrix32F> decompQRP = DecompositionFactory.qrp(500, 100); // todo this should be passed in
    BidiagonalDecomposition<DenseMatrix32F> decompBi = new BidiagonalDecompositionRow_D32();

    DenseMatrix32F B = new DenseMatrix32F(1,1);
    ...
    public DenseMatrix32F getB(DenseMatrix32F B, boolean compact) {
        B = BidiagonalDecompositionRow_D32.handleB(B, compact, m, n, min);
    ...
    public DenseMatrix32F getU(DenseMatrix32F U, boolean transpose, boolean compact) {
        U = BidiagonalDecompositionRow_D32.handleU(U, false, compact, m, n, min);

        if( compact ) {
            // U = Q*U1
            DenseMatrix32F Q1 = decompQRP.getQ(null,true);
            DenseMatrix32F U1 = decompBi.getU(null,false,true);
            CommonOps.mult(Q1,U1,U);
        } else {
           // U = [Q1*U1 Q2]
            DenseMatrix32F Q = decompQRP.getQ(U,false);
            DenseMatrix32F U1 = decompBi.getU(null,false,true);
            DenseMatrix32F Q1 = CommonOps.extract(Q,0,Q.numRows,0,min);
            DenseMatrix32F tmp = new DenseMatrix32F(Q1.numRows,U1.numCols);
    ...
    public boolean decompose(DenseMatrix32F orig) {
        ...
        DenseMatrix32F result = new DenseMatrix32F(min,n);
        DenseMatrix32F P = decompQRP.getPivotMatrix(null);
