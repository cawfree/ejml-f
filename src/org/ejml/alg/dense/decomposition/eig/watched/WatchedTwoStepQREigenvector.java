...
public class WatchedTwoStepQREigenvector {
    ...
    WatchedTwoStepQREigen implicit;

    // Q matrix from float step QR
    DenseMatrix32F Q;


    DenseMatrix32F eigenvectors[];

    DenseMatrix32F eigenvectorTemp;

    LinearSolver solver;

    Complex32F origEigenvalues[];
    int N;
    ...
    public boolean process( WatchedTwoStepQREigen implicit , DenseMatrix32F A , DenseMatrix32F Q_h )
    {
        this.implicit = implicit;

        if( N != A.numRows ) {
            N = A.numRows;
            Q = new DenseMatrix32F(N,N);
            splits = new int[N];
            origEigenvalues = new Complex32F[N];
            eigenvectors = new DenseMatrix32F[N];
            eigenvectorTemp = new DenseMatrix32F(N,1);
...
//        A.print("%12.10f");
    ...
    public boolean extractVectors( DenseMatrix32F Q_h ) {
        ...
        for( int i = 0; i < N; i++ ) {
            ...
            Complex32F c = implicit.eigenvalues[N-i-1];
        ...
        if( Q_h != null ) {
            DenseMatrix32F temp = new DenseMatrix32F(N,1);
            for( int i = 0; i < N; i++ ) {
                DenseMatrix32F v = eigenvectors[i];
    ...
    private void solveEigenvectorDuplicateEigenvalue( float real , int first , boolean isTriangle ) {
        ...
        for( int i = first; i < N; i++ ) {
            ...
            if( c.isReal() && Math.abs(c.real-real)/scale < 100.0*UtilEjml.EPS ) {
                ...
                DenseMatrix32F v = new DenseMatrix32F(N,1);
    ...
    private void solveUsingTriangle(float real, int index, DenseMatrix32F r ) {
    ...
    private void solveWithLU(float real, int index, DenseMatrix32F r ) {
        DenseMatrix32F A = new DenseMatrix32F(index,index);
...
//                return false;
    ...
    private void checkSplitPerformImplicit() {
        ...
        if( onscript) {
            ...
            if( x2-x1 >= 1 && x2 < N-2 )
                implicit.implicitFloatStep(x1,x2);
            else
