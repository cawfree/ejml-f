...
public class WatchedTwoStepQREigen {
    ...
    public void setQ( DenseMatrix32F Q ) {
        this.Q = Q;
    }
    ...
    public void setup( DenseMatrix32F A ) {
        ...
        if( N != A.numRows ) {
            ...
            this.A = A.copy();
            u = new DenseMatrix32F(A.numRows,1);

            _temp = new DenseMatrix32F(A.numRows,1);
        ...
        eigenvalues = new Complex32F[ A.numRows ];
        for( int i = 0; i < eigenvalues.length; i++ ) {
            eigenvalues[i] = new Complex32F();
        }
    ...
    public void exceptionalShift( int x1 , int x2) {
        ...
        float p = 1-(float)Math.pow(0.1,numExceptional);
        val *= p+2.0f*(1.0f-p)*(rand.nextFloat()-0.5);
     ...
     * @param x2
     */
    public void implicitFloatStep( int x1 , int x2 ) {
        if( printHumps )
        ...
        performImplicitfloatStep(x1, x2, b11 , b21 , b31 );
    }
     ...
     */
    public void performImplicitfloatStep(int x1, int x2 , float real , float img  ) {
        float a11 = A.get(x1,x1);
        float a21 = A.get(x1+1,x1);
        float a12 = A.get(x1,x1+1);
        float a22 = A.get(x1+1,x1+1);
    ...
    private void performImplicitfloatStep(int x1, int x2,
                                           float b11 , float b21 , float b31 ) {
        if( !bulgefloatStepQn(x1,b11,b21,b31,0,false) )
            return;

        // get rid of the bump
        if( Q != null ) {
            QrHelperFunctions.rank1UpdateMultR(Q,u.data,gamma,0,x1,x1+3,_temp.data);
            if( checkOrthogonal && !MatrixFeatures.isOrthogonal(Q,1e-8f) ) {
                u.print();
...
//        A.print("%12.3e");
        ...
        if( checkHessenberg && !MatrixFeatures.isUpperTriangle(A,1,1e-12f)) {
            A.print("%12.3e");
    ...
    public boolean createBulgeSingleStep( int x1 , float eigenvalue ) {
        ...
        float threshold = Math.abs(A.get(x1,x1))*UtilEjml.EPS;

        return bulgeSingleStepQn(x1,b11,b21,threshold,false);
    }

    public boolean bulgefloatStepQn( int i ) {
        float a11 = A.get(i+1,i);
        float a21 = A.get(i+2,i);
        float a31 = A.get(i+3,i);

        float threshold = Math.abs(A.get(i,i))*UtilEjml.EPS;

        return bulgefloatStepQn(i+1,a11,a21,a31,threshold,true);
    }
