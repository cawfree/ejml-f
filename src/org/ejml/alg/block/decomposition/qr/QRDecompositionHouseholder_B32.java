...
/**
 * <p>
 * QR decomposition for {@link BlockMatrix32F} using householder reflectors.  The decomposition is
 * performed by computing a QR decomposition for each block column as is normally done, see {@link org.ejml.alg.dense.decomposition.qr.QRDecompositionHouseholder_D32}.
 ...
 * @author Peter Abeles
 */
public class QRDecompositionHouseholder_B32
        implements QRDecomposition<BlockMatrix32F> {
    ...
    private D1Submatrix32F W = new D1Submatrix32F(dataW);
    private D1Submatrix32F WTA = new D1Submatrix32F(dataWTA);
     ...
     */
    @Override
    public BlockMatrix32F getQ(BlockMatrix32F Q, boolean compact) {
     ...
     */
    public static BlockMatrix32F initializeQ(BlockMatrix32F Q,
                                              int numRows , int numCols , int blockLength ,
                                              boolean compact) {
        int minLength = Math.min(numRows,numCols);
        if( compact ) {
            if( Q == null ) {
                Q = new BlockMatrix32F(numRows,minLength,blockLength);
            ...
            if( Q == null ) {
                Q = new BlockMatrix32F(numRows,numRows,blockLength);
                BlockMatrixOps.setIdentity(Q);
            } else {
     ...
     */
    public void applyQ( BlockMatrix32F B ) {
        applyQ(B,false);
    }
     ...
     */
    public void applyQ( BlockMatrix32F B , boolean isIdentity ) {
        int minDimen = Math.min(dataA.numCols,dataA.numRows);

        D1Submatrix32F subB = new D1Submatrix32F(B);
     ...
     * @param B Matrix which Q is applied to.  Modified.
     */
    public void applyQTran( BlockMatrix32F B ) {
        int minDimen = Math.min(dataA.numCols,dataA.numRows);
...
//            W.original.reshape(W.row1,W.col1,false);
     ...
     */
    @Override
    public BlockMatrix32F getR(BlockMatrix32F R, boolean compact) {
        int min = Math.min(dataA.numRows,dataA.numCols);

        if( R == null ) {
            if( compact ) {
                R = new BlockMatrix32F(min,dataA.numCols,blockLength);
            } else {
                R = new BlockMatrix32F(dataA.numRows,dataA.numCols,blockLength);
            }
        } else {
            if( compact ) {
     ...
     * @param orig
     */
    private void setup(BlockMatrix32F orig) {
     ...
     * </p>
     */
    protected void updateA( D1Submatrix32F A )
    {
        setW();
