...
public class TridiagonalHelper_B32 {
    ...
    public static void computeW_row( final int blockLength ,
        ...
        for( int i = 1; i < min; i++ ) {
            ...
            // w = w -beta*W*(Y^T*u)
            for( int j = 0; j < i; j++ ) {
                float yv = BlockHouseHolder.innerProdRow(blockLength,Y,i,Y,j,1);
                                 ...
                                 float gamma )
    {
        ...
        for( int i = 0; i < row; i++ ) {
            ...
            // v_i^t*u
            float dot_v_u = BlockHouseHolder.innerProdRow(blockLength,A,row,V,i,1);

            // u_i^t*u
            float dot_u_u = BlockHouseHolder.innerProdRow(blockLength,A,row,A,i,1);
    ...
    {
        // val=(y^T*u)
        float val = BlockHouseHolder.innerProdRow(blockLength,A,row,V,row,1);
        ...
        // v = y - (1/2)gamma*val * u
        BlockVectorOps.add_row(blockLength,V,row,1,A,row,-0.5f*gamma*val,V,row,row+1,A.col1-A.col0);
