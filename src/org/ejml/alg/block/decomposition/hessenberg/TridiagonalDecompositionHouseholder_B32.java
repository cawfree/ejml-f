 ...
 * @author Peter Abeles
 */
public class TridiagonalDecompositionHouseholder_B32
        implements TridiagonalSimilarDecomposition<BlockMatrix32F> {

    // matrix which is being decomposed
    // householder vectors are stored along the upper triangle rows
    protected BlockMatrix32F A;
    // temporary storage for block computations
    protected BlockMatrix32F V = new BlockMatrix32F(1,1);
    // stores intermediate results in matrix multiplication
    protected BlockMatrix32F tmp = new BlockMatrix32F(1,1);
    protected float gammas[] = new float[1];

    // temporary storage for zeros and ones in U
    protected DenseMatrix32F zerosM = new DenseMatrix32F(1,1);

    @Override
    public BlockMatrix32F getT(BlockMatrix32F T) {
        if( T == null ) {
            T = new BlockMatrix32F(A.numRows,A.numCols,A.blockLength);
        } else {
    ...
    @Override
    public BlockMatrix32F getQ(BlockMatrix32F Q, boolean transposed) {
        Q = QRDecompositionHouseholder_B32.initializeQ(Q, A.numRows, A.numCols, A.blockLength, false);

        int height = Math.min(A.blockLength,A.numRows);
        V.reshape(height,A.numCols,false);
        tmp.reshape(height,A.numCols,false);

        D1Submatrix32F subQ = new D1Submatrix32F(Q);
        D1Submatrix32F subU = new D1Submatrix32F(A);
        D1Submatrix32F subW = new D1Submatrix32F(V);
        D1Submatrix32F tmp = new D1Submatrix32F(this.tmp);
        ...
        for( int i = start; i >= 0; i -= A.blockLength ) {
            ...
            TridiagonalHelper_B32.computeW_row(A.blockLength, subU, subW, gammas, i);
    ...
    private void copyZeros( D1Submatrix32F subU ) {
    ...
    private void replaceZeros( D1Submatrix32F subU ) {
        int N = Math.min(A.blockLength,subU.col1-subU.col0);
        for( int i = 0; i < N; i++ ) {
            // save the zeros
            for( int j = 0; j <= i; j++ ) {
    ...
    @Override
    public boolean decompose(BlockMatrix32F orig) {
        if( orig.numCols != orig.numRows )
            throw new IllegalArgumentException("Input matrix must be square.");

        init(orig);

        D1Submatrix32F subA = new D1Submatrix32F(A);
        D1Submatrix32F subV = new D1Submatrix32F(V);
        D1Submatrix32F subU = new D1Submatrix32F(A);
...
//            System.out.println("-------- triag i "+i);
            ...
            TridiagonalHelper_B32.tridiagUpperRow(A.blockLength, subA, gammas, subV);
    ...
    private void init( BlockMatrix32F orig ) {
        this.A = orig;
