...
public class SpecializedOps {
     ...
     *
     * <p>
     * In practice {@link org.ejml.alg.dense.mult.VectorVectorMult#householder(float, org.ejml.data.D1Matrix32F, org.ejml.data.D1Matrix32F, org.ejml.data.D1Matrix32F)}  multHouseholder}
     ...
     */
    public static DenseMatrix32F createReflector( RowD1Matrix32F u ) {
        if( !MatrixFeatures.isVector(u))
            throw new IllegalArgumentException("u must be a vector");

        float norm = NormOps.fastNormF(u);
        float gamma = -2.0f/(norm*norm);

        DenseMatrix32F Q = CommonOps.identity(u.getNumElements());
     ...
     * @return An orthogonal reflector.
     */
    public static DenseMatrix32F createReflector( DenseMatrix32F u , float gamma) {
        if( !MatrixFeatures.isVector(u))
            throw new IllegalArgumentException("u must be a vector");
     ...
     */
    public static DenseMatrix32F copyChangeRow( int order[] , DenseMatrix32F src , DenseMatrix32F dst )
    {
        if( dst == null ) {
     ...
     */
    public static DenseMatrix32F copyTriangle( DenseMatrix32F src , DenseMatrix32F dst , boolean upper ) {
        if( dst == null ) {
            dst = new DenseMatrix32F(src.numRows,src.numCols);
        } else if( src.numRows != dst.numRows || src.numCols != dst.numCols ) {
            throw new IllegalArgumentException("src and dst must have the same dimensions.");
     ...
     */
    public static float diffNormF( D1Matrix32F a , D1Matrix32F b )
    {
        ...
        DenseMatrix32F diff = new DenseMatrix32F(size,1);
    ...
    public static float diffNormF_fast( D1Matrix32F a , D1Matrix32F b )
    {
        if( a.numRows != b.numRows || a.numCols != b.numCols ) {
            throw new IllegalArgumentException("Both matrices must have the same shape.");
        }
     ...
     */
    public static float diffNormP1( D1Matrix32F a , D1Matrix32F b )
    {
        if( a.numRows != b.numRows || a.numCols != b.numCols ) {
            throw new IllegalArgumentException("Both matrices must have the same shape.");
        }
     ...
     */
    public static void addIdentity( RowD1Matrix32F A , RowD1Matrix32F B , float alpha )
    {
     ...
     */
    public static void subvector(RowD1Matrix32F A, int rowA, int colA, int length , boolean row, int offsetV, RowD1Matrix32F v) {
        if( row ) {
     ...
     */
    public static DenseMatrix32F[] splitIntoVectors( RowD1Matrix32F A , boolean column )
    {
        ...
        DenseMatrix32F[] ret  = new DenseMatrix32F[w];

        for( int i = 0; i < w; i++ ) {
            DenseMatrix32F a = new DenseMatrix32F(M,N);
     ...
     */
    public static DenseMatrix32F pivotMatrix(DenseMatrix32F ret, int pivots[], int numPivots, boolean transposed ) {

        if( ret == null ) {
            ret = new DenseMatrix32F(numPivots, numPivots);
        } else {
     ...
     */
    public static float diagProd( RowD1Matrix32F T )
    {
        float prod = 1.0f;
     ...
     */
    public static float qualityTriangular(boolean upper, D1Matrix32F T)
    {
        int N = Math.min(T.numRows,T.numCols);

        // TODO make faster by just checking the upper triangular portion
        float max = CommonOps.elementMaxAbs(T);
     ...
     */
    public static float elementSumSq( D1Matrix32F m  ) {
        float total = 0;
