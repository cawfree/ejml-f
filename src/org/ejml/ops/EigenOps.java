...
public class EigenOps {
     ...
     */
    public static float computeEigenValue( DenseMatrix32F A , DenseMatrix32F eigenVector )
    {
        float bottom = VectorVectorMult.innerProd(eigenVector,eigenVector);
        float top = VectorVectorMult.innerProdA(eigenVector,A,eigenVector);
     ...
     */
    public static Eigenpair computeEigenVector( DenseMatrix32F A , float eigenvalue )
    {
        if( A.numRows != A.numCols )
            throw new IllegalArgumentException("Must be a square matrix.");

        DenseMatrix32F M = new DenseMatrix32F(A.numRows,A.numCols);

        DenseMatrix32F x = new DenseMatrix32F(A.numRows,1);
        DenseMatrix32F b = new DenseMatrix32F(A.numRows,1);
...
//        eigenvalue -= eigenvalue*UtilEjml.EPS*10;
        ...
        float threshold = NormOps.normPInf(A)*UtilEjml.EPS;

        float prevError = Float.MAX_VALUE;
        boolean hasWorked = false;

        LinearSolver<DenseMatrix32F> solver = LinearSolverFactory.linear(M.numRows);
        ...
        for( int i = 0; i < 200; i++ ) {
            ...
            if( failed ) {
                ...
                float error = NormOps.normPInf(x);
...
//                    CommonOps.set(b,1);
    ...
    public static Eigenpair dominantEigenpair( DenseMatrix32F A ) {
     ...
     */
    public static float [] boundLargestEigenValue( DenseMatrix32F A , float []bound ) {
        if( A.numRows != A.numCols )
     ...
     */
    public static DenseMatrix32F createMatrixD( EigenDecomposition eig )
    {
        int N = eig.getNumberOfEigenvalues();

        DenseMatrix32F D = new DenseMatrix32F( N , N );

        for( int i = 0; i < N; i++ ) {
            Complex32F c = eig.getEigenvalue(i);
     ...
     */
    public static DenseMatrix32F createMatrixV( EigenDecomposition<DenseMatrix32F> eig )
    {
        int N = eig.getNumberOfEigenvalues();

        DenseMatrix32F V = new DenseMatrix32F( N , N );

        for( int i = 0; i < N; i++ ) {
            Complex32F c = eig.getEigenvalue(i);

            if( c.isReal() ) {
                DenseMatrix32F v = eig.getEigenVector(i);
