...
public class NormOps {
     ...
     */
    public static void normalizeF( DenseMatrix32F A ) {
        float val = normF(A);
     ...
     */
    public static float conditionP( DenseMatrix32F A , float p )
    {
        if( p == 2 ) {
            ...
            DenseMatrix32F A_inv = new DenseMatrix32F(A.numRows,A.numCols);
            ...
            DenseMatrix32F pinv = new DenseMatrix32F(A.numCols,A.numRows);
     ...
     * @return The condition number.
     */
    public static float conditionP2( DenseMatrix32F A )
    {
        ...
        int n = SingularOps.rank(svd,1e-12f);
     ...
     */
    public static float fastNormF( D1Matrix32F a ) {
        float total = 0;
     ...
     */
    public static float normF( D1Matrix32F a ) {
        float total = 0;

        float scale = CommonOps.elementMaxAbs(a);
     ...
     * @return The norm's value.
     */
    public static float elementP( RowD1Matrix32F A , float p ) {
        if( p == 1 ) {
            return CommonOps.elementSumAbs(A);
        } if( p == 2 ) {
            return normF(A);
        } else {
            float max = CommonOps.elementMaxAbs(A);
            ...
            return max*(float)Math.pow(total,1.0/p);
        }
    }
     ...
     * @return The norm's value.
     */
    public static float fastElementP( D1Matrix32F A , float p ) {
        if( p == 2 ) {
            return fastNormF(A);
        } else {
            float total = 0;
            ...
            return (float)Math.pow(total,1.0/p);
        }
    }
     ...
     */
    public static float normP( DenseMatrix32F A , float p ) {
        if( p == 1 ) {
            return normP1(A);
        } else if( p == 2 ) {
            return normP2(A);
     ...
     * @return The computed norm.
     */
    public static float fastNormP( DenseMatrix32F A , float p ) {
        if( p == 1 ) {
            return normP1(A);
        } else if( p == 2 ) {
            return fastNormP2(A);
     ...
     * @return The norm.
     */
    public static float normP1( DenseMatrix32F A ) {
        if( MatrixFeatures.isVector(A)) {
            return CommonOps.elementSumAbs(A);
        } else {
            return inducedP1(A);
     ...
     * @return The norm.
     */
    public static float normP2( DenseMatrix32F A ) {
        if( MatrixFeatures.isVector(A)) {
            return normF(A);
        } else {
            return inducedP2(A);
     ...
     * @return The norm.
     */
    public static float fastNormP2( DenseMatrix32F A ) {
        if( MatrixFeatures.isVector(A)) {
            return fastNormF(A);
        } else {
            return inducedP2(A);
     ...
     * @return The norm.
     */
    public static float normPInf( DenseMatrix32F A ) {
        if( MatrixFeatures.isVector(A)) {
     ...
     * @return The norm.
     */
    public static float inducedP1( DenseMatrix32F A ) {
        float max = 0;
     ...
     * @return The norm.
     */
    public static float inducedP2( DenseMatrix32F A ) {
        SingularValueDecomposition<DenseMatrix32F> svd = DecompositionFactory.svd(A.numRows,A.numCols,false,false,true);
     ...
     * @return the norm.
     */
    public static float inducedPInf( DenseMatrix32F A ) {
        float max = 0;
