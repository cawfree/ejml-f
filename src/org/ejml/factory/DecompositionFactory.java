...
public class DecompositionFactory {
     ...
     */
    public static CholeskyDecomposition<DenseMatrix32F> chol( int matrixSize , boolean lower )
    {
        if( matrixSize < EjmlParameters.SWITCH_BLOCK64_CHOLESKY ) {
            return new CholeskyDecompositionInner_D32(lower);
        } else if( EjmlParameters.MEMORY == EjmlParameters.MemoryUsage.FASTER ){
            return new CholeskyDecomposition_B32_to_D32(lower);
        } else {
            return new CholeskyDecompositionBlock_D32(EjmlParameters.BLOCK_WIDTH_CHOL);
        }
    }

    /**
     * <p>
     * Returns a {@link org.ejml.alg.dense.decomposition.chol.CholeskyDecompositionLDL_D32} that has been optimized for the specified matrix size.
     * </p>
     *
     * @param matrixSize Number of rows and columns that the returned decomposition is optimized for.
     * @return CholeskyLDLDecomposition
     */
    public static CholeskyLDLDecomposition<DenseMatrix32F> cholLDL( int matrixSize ) {
     ...
     */
    public static LUDecomposition<DenseMatrix32F> lu( int numRows , int numCol ) {
     ...
     * @return
     */
    public static SingularValueDecomposition<DenseMatrix32F> svd( int numRows , int numCols , 
                                                                  boolean needU , boolean needV , boolean compact ) {
        // Don't allow the tall decomposition by default since it *might* be less stable
        return new SvdImplicitQrDecompose_D32(compact,needU,needV,false);
    }
     ...
     */
    public static QRDecomposition<DenseMatrix32F> qr( int numRows , int numCols ) {
     ...
     */
    public static QRPDecomposition<DenseMatrix32F> qrp( int numRows , int numCols ) {
     ...
     */
    public static EigenDecomposition<DenseMatrix32F> eig( int matrixSize , boolean needVectors ) {
        return new SwitchingEigenDecomposition(matrixSize,needVectors,1e-8f);
    }
     ...
     */
    public static EigenDecomposition<DenseMatrix32F> eig( int matrixSize , boolean computeVectors ,
                                                          boolean isSymmetric ) {
        if( isSymmetric ) {
            TridiagonalSimilarDecomposition<DenseMatrix32F> decomp = DecompositionFactory.tridiagonal(matrixSize);
            return new SymmetricQRAlgorithmDecomposition_D32(decomp,computeVectors);
        } else
            return new WatchedTwoStepQRDecomposition_D32(computeVectors);
    }
     ...
     */
    public static float quality( DenseMatrix32F orig , SingularValueDecomposition<DenseMatrix32F> svd )
    {
        return quality(orig,svd.getU(null,false),svd.getW(null),svd.getV(null,true));
    }

    public static float quality( DenseMatrix32F orig , DenseMatrix32F U , DenseMatrix32F W , DenseMatrix32F Vt )
    {
     ...
     * @return The quality of the decomposition.
     */
    public static float quality( DenseMatrix32F orig , EigenDecomposition<DenseMatrix32F> eig )
    {
        ...
        float top = diff.normF();
        float bottom = L.normF();
     ...
     * @param matrixSize Number of rows and columns that the returned decomposition is optimized for.
     */
    public static TridiagonalSimilarDecomposition<DenseMatrix32F> tridiagonal(  int matrixSize ) {
        if( matrixSize >= 1800 ) {
            return new TridiagonalDecomposition_B32_to_D32();
        } else {
            return new TridiagonalDecompositionHouseholder_D32();
        }
    }
     ...
     */
    public static <T extends ReshapeMatrix32F> boolean decomposeSafe( DecompositionInterface<T> decomp, T M ) {
